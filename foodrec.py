...
def get_restaurants_in_category(category: CategoryInfo, restaurants_limit: Optional[int]) -> List[RestaurantInfo]:
...
def get_restaurant_info(restaurant: RestaurantInfo, browser: webdriver.Chrome, executor: concurrent.futures.ThreadPoolExecutor, items_limit: Optional[int], sleep_sec: Optional[int]) -> RestaurantInfo:
...
def get_restaurants(city: str, browser: webdriver.Chrome, executor: concurrent.futures.ThreadPoolExecutor, 
categories_limit: Optional[int] = None, restaurants_limit: Optional[int] = None, items_limit: Optional[int] = None, sleep_sec: Optional[int] = None) -> List[RestaurantInfo]:
    restaurants_csv_path = Path(f'restaurants-info-{parse_city(city)}.csv')
    categories_info = get_categories_info_for_city(city)[:categories_limit] if categories_limit is not None else get_categories_info_for_city(city)

    for category in tqdm(categories_info):
        restaurants = get_restaurants_in_category(category, restaurants_limit=restaurants_limit)
        for i in tqdm(range(len(restaurants))):
            restaurants[i] = get_restaurant_info(restaurants[i], browser, executor, items_limit, sleep_sec)
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        for city in cities:
            get_restaurants(city, setup_browser(), executor, categories_limit=None, restaurants_limit=None, items_limit=10, sleep_sec=1)